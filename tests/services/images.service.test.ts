...
describe('image.service', () => {
  ...
  it('debería retornar una lista de imágenes por breed_id', async () => {
    const mockImages: CatImage[] = [
      { id: 'img1', url: 'https://example.com/img1.jpg', height: 150, width: 200 },
      { id: 'img2', url: 'https://example.com/img2.jpg', height: 150, width: 200 }
    ];

    const apiGetMock = jest.spyOn(ImageService.api, 'get').mockResolvedValue({ data: mockImages });
  ...
  it('debería lanzar un error si axios falla', async () => {
    ...
    await expect(ImageService.getImagesByBreedId('abys')).rejects.toThrow('API error');
  });
});
