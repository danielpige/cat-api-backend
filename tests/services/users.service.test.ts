...
describe('user.service', () => {
  ...
  describe('registerUser', () => {
    it('debería registrar un nuevo usuario correctamente', async () => {
      const newUser = { username: 'testuser', email: 'test@example.com', password: '123456' };
  ...
  describe('loginUser', () => {
    ...
    it('debería loguear correctamente si la contraseña es válida y el usuario está habilitado', async () => {
      ...
      expect(UserModel.findOne).toHaveBeenCalledWith({ username: loginData.username });
    ...
    it('debería lanzar error si el usuario no existe', async () => {
      ...
      await expect(userService.loginUser(loginData)).rejects.toThrow('Usuario o contraseña incorrecto.');
    });
    ...
    it('debería lanzar error si el usuario está deshabilitado', async () => {
      ...
      await expect(userService.loginUser(loginData)).rejects.toThrow('Usuario deshabilitado');
    });
  });
});
