...
import app from "../../app";
import * as CatsController from "../../controllers/cats.controller";

const JWT_SECRET = process.env.JWT_SECRET || "default_secret_key";

jest.mock("../../controllers/cats.controller.ts", () => ({
  getBreeds: jest.fn((req, res) =>
    res.status(200).json({ breeds: ["siamese", "persian"] })
  ),
  searchBreeds: jest.fn((req, res) =>
    res.status(200).json({ results: ["bengal"] })
  ),
  getBreedById: jest.fn((req, res) =>
    res.status(200).json({ breed: { id: "beng", name: "Bengal" } })
  ),
}));
